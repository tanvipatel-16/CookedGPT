...
function playSend() { playTone(600,'sine',.12,.12); setTimeout(()=>playTone(800,'sine',.1,.08),80); }
...
function playSelect() { playTone(500,'sine',.15,.1); setTimeout(()=>playTone(700,'sine',.1,.08),100); }
...
function spawnParticles(x, y) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    p.style.cssText = `left:${x + (Math.random()-0.5)*60}px;top:${y + (Math.random()-0.5)*60}px;animation-delay:${Math.random()*0.2}s;font-size:${0.8+Math.random()*0.8}rem;`;
...
function initMoods() {
  ...
  function updatePanel(key) {
    ...
    const avatarEl = document.getElementById('panel-emoji');
    if (avatarEl) { avatarEl.textContent = m.emoji; avatarEl.style.animation='none'; setTimeout(()=>avatarEl.style.animation='',50); }
    const nameEl = document.getElementById('panel-name');
    if (nameEl) nameEl.textContent = m.name;
    const traitsEl = document.getElementById('panel-traits');
    if (traitsEl) traitsEl.innerHTML = m.traits.map(t=>`<span class="trait-badge">${t}</span>`).join('');
    ...
    if (panel) { panel.style.animation='none'; setTimeout(()=>{panel.style.animation='fadeUp .4s ease';},10); }
  }

  function updateSlider(val) {
    const pct = ((val-1)/9)*100;
    if (slider) slider.style.background = `linear-gradient(to right,#7c3aed 0%,#a855f7 ${pct}%,#2d2d3d ${pct}%,#2d2d3d 100%)`;
    if (valDisplay) valDisplay.childNodes[0].textContent = val;
    if (levelDisplay) levelDisplay.textContent = INTENSITY_LABELS[val] || 'MAX';
  }

  function selectMood(key, fromClick, event) {
    selectedMood = key;
    cards.forEach(c => {
      const isSelected = c.dataset.mood === key;
      c.classList.toggle('selected', isSelected);
      const ch = c.querySelector('.check-badge');
      if (ch) ch.style.display = isSelected ? 'flex' : 'none';
    });
    updatePanel(key);
    Object.keys(MOODS).forEach(k => buildDangerBars(k, `danger-${k}`));
    if (fromClick) {
      playSelect();
      if (event) spawnParticles(event.clientX, event.clientY);
    }
  }

  cards.forEach(card => {
    card.addEventListener('click', (e) => selectMood(card.dataset.mood, true, e));
  });
...
function initChat() {
  ...
  const moodKey = params.get('mood') || 'savage';
  const intensity = parseInt(params.get('intensity')) || 7;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (path === '/moods' || path.includes('index')) initMoods();
